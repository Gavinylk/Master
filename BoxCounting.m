function [D] = BoxCounting(binaryImg)

% Haussdorf fractal dimension of a binary image
...
maxDim = max(size(binaryImg));
    newDimSize = 2^ceil(log2(maxDim));
    rowPad = newDimSize - size(binaryImg, 1);
    colPad = newDimSize - size(binaryImg, 2);
    binaryImg = padarray(binaryImg, [rowPad, colPad], 'post');

    boxCounts = zeros(1, ceil(log2(maxDim)));
    resolutions = zeros(1, ceil(log2(maxDim)));
    
    iSize = size(binaryImg, 1);
    boxSize = iSize;
    boxesPerDim = 1;
    idx = 0;
    while boxSize >= 1
